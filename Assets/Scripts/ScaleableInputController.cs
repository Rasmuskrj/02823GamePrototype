...
public class ScaleableInputController : MonoBehaviour {
	...
	void Update () {
        ...
        {
            if (Input.GetAxisRaw(xControlNames[i]) != 0) { game[i].MoveX(Input.GetAxis(xControlNames[i]));
                if (x_isAxisInUse[i] == false) { x_isAxisInUse[i] = true; game[i].MoveXRaw(Input.GetAxisRaw(xControlNames[i])); } }
